...
Usage
-----
    python visualize_tone_event.py \
...
def build_payload(
    ...
    # GENOME per-event fields for the intensity-vs-valence view (optional).
    # intensity comes from the events CSV; category may be there or in the toned file.
    ...
    if events_df is not None and "id" in events_df.columns:
        ...
        if "intensity" in e2.columns:
            for _, r in e2.iterrows():
                v = pd.to_numeric(r["intensity"], errors="coerce")
                if pd.notna(v):
                    ev_intensity[r["id"]] = float(v)
        if "category" in e2.columns:
            for _, r in e2.iterrows():
                ev_category[r["id"]] = str(r["category"])
        for _, r in e2.iterrows():
            det = {}
            for f in _DETAIL_FIELDS:
                if f in e2.columns:
                    v = r[f]
                    det[f] = "" if pd.isna(v) else str(v)
            ev_detail[r["id"]] = det
    ...
    for eid, g in df.groupby("event_id"):
        ...
        ev_meta[eid] = {
            ...
            "category": cat or "",
            "detail": ev_detail.get(eid, {}),     # full GENOME record if events file given
        }
    ...
    return {
        ...
        "hasIntensity": any(e.get("intensity") is not None for e in events),
...
def visualize_tone(
    ...
    if not payload["hasIntensity"]:
        print("[note] no event intensity found — pass --events events.csv to enable "
              "the GENOME intensity-vs-valence view.", file=sys.stderr)
...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .evcard .ctag{font-size:11px;font-weight:600;padding:3px 10px;border-radius:20px;letter-spacing:.4px;
  ...
  .evcard .quote{font-style:italic;color:#b9c6da}
...
<body><div class="wrap">
  ...
  <div class="grid" id="grid">
    ...
    <div class="panel full" id="genomePanel"><h2>GENOME view — public tone vs coded intensity</h2>
      <div class="hint" id="genomehint">One bubble per event · x = GENOME coded intensity, y = public mean valence, size = upvote engagement. Always shows all events; subreddit filter applies.</div>
      <canvas id="genome"></canvas></div>
...
// GENOME bubble scatter: one point per event (intensity vs valence), size = engagement.
// Hidden entirely when no event intensity was supplied (no --events).
let genomeCh = null;
const catColor = c => /CONFLICT/i.test(c) ? "#e5484d" : /COOP/i.test(c) ? "#30a46c" : "#7c8695";
if(D.hasIntensity){
  genomeCh = new Chart(genome,{type:"bubble",
    data:{datasets:[
      {label:"Events", data:[], parsing:false},
      {type:"line", label:"crowd = coded", data:[{x:-1,y:-1},{x:1,y:1}],
       borderColor:"rgba(255,255,255,.25)", borderDash:[6,5], borderWidth:1,
       pointRadius:0, fill:false}]},
    options:{plugins:{legend:{display:false},
      tooltip:{callbacks:{label:c=>{const m=c.raw.m; return m ? [
        `${(m.type||"event").toUpperCase()} · ${m.date}`,
        `coded intensity: ${m.intensity}`,
        `public valence: ${m.val.toFixed(2)}  (gap ${(m.val-m.intensity>=0?"+":"")+(m.val-m.intensity).toFixed(2)})`,
        `dominant: ${CAP(m.dom)} · ${m.n} comments · ▲${m.eng}`] : c.formattedValue;}}}},
      scales:{
        x:{min:-1,max:1,title:{display:true,text:"GENOME coded intensity (−1 conflict … +1 cooperation)"},grid:{color:"rgba(255,255,255,.06)"}},
        y:{min:-1,max:1,title:{display:true,text:"Public mean valence"},grid:{color:"rgba(255,255,255,.06)"}}}}});
} else {
  document.getElementById("genomePanel").style.display = "none";
}
...
function render(){
  ...
  if(idx.length===0){ grid.style.display="none"; empty.style.display="block";
    document.getElementById("cards").innerHTML=""; updateScope(0,nDead); renderEventCard(); return; }
...
// GENOME event detail card (top). Shows the selected event's full record from
// the events CSV; when All events is selected, shows a short overview prompt.
function renderEventCard(){
  ...
  if(selEvent<0){
    el.innerHTML = `<div class="evprompt">Showing <b style="color:var(--ink)">all ${D.events.length} events</b>. `
      + (D.hasIntensity ? "Pick an event above to see its full GENOME record here, or explore the GENOME view below."
                        : "Pass --events to load the GENOME records.") + `</div>`;
    return;
  }
  const e = D.events[selEvent], d = e.detail || {};
  const conflict = /CONFLICT/i.test(e.category);
  const etagColor = conflict ? "#e5484d" : /COOP/i.test(e.category) ? "#30a46c" : "#7c8695";
  ...
  el.innerHTML = `
    ...
    ${sec("Source quote", d.source_quote, "quote")}
    <div class="agrid">
...
// GENOME view: aggregate per event across all events, honouring only the
// subreddit filter (event/post drilldown doesn't apply to a cross-event scatter).
function renderGenome(){
  if(!genomeCh) return;
  const E = EMO.length;
  const acc = new Map();   // eventIdx -> {valSum,n,eng}
  for(let i=0;i<N;i++){
    if(selSub>=0 && C.si[i]!==selSub) continue;
    const ev = C.ei[i]; if(ev<0) continue;
    let a = acc.get(ev); if(!a){ a={valSum:0,n:0,eng:0,dom:new Array(E).fill(0)}; acc.set(ev,a); }
    const pr=C.pr[i]; let v=0; for(let e=0;e<E;e++) v+=pr[e]*VAL[e];
    a.valSum+=v; a.n++; a.eng+=Math.max(C.sc[i],0); a.dom[C.top[i]]++;
  }
  // build points; AREA tracks engagement, but with a generous floor so every
  // event is clearly visible even when one event dominates the upvotes (or when
  // engagement is zero everywhere).
  let maxEng=0; acc.forEach(a=>{ if(a.eng>maxEng) maxEng=a.eng; });
  const pts=[];
  acc.forEach((a,ev)=>{
    const meta=D.events[ev]; if(meta.intensity===null||meta.intensity===undefined) return;
    const val=a.valSum/a.n, dom=EMO[a.dom.indexOf(Math.max(...a.dom))];
    const frac = maxEng>0 ? Math.sqrt(a.eng/maxEng) : 0;   // 0..1
    pts.push({x:meta.intensity, y:val, r:10+22*frac,
      backgroundColor:catColor(meta.category), borderColor:"#e8eef7", borderWidth:2,
      m:{type:meta.type,date:meta.date,intensity:meta.intensity,val,dom,n:a.n,eng:a.eng}});
  });
  genomeCh.data.datasets[0].data = pts;
  genomeCh.update();
  document.getElementById("genomehint").textContent =
    `One bubble per event · x = coded intensity, y = public valence, size = upvote engagement, colour = category`
    + (selSub>=0 ? ` · r/${D.subs[selSub]} only` : "") + " · points above the dashed line are warmer than coded";
}
...
def main() -> None:
    ...
    #                 help="GENOME events CSV (enables the intensity-vs-valence view)")
    ...
    # args = ap.parse_args()
    visualize_tone("data/tone_comments/new_model_2022_02_tone_comments.csv","event_tone_dashboard.html",
                   events_path="data/events/EVENTS_2022_02.csv")
